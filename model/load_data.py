...
images: list[str] = sorted(
    glob(os.path.join(DATA_DIR, "Images/*")))[:NUM_TRAIN_IMAGES + NUM_VAL_IMAGES]
masks: list[str] = sorted(
    glob(os.path.join(DATA_DIR, "Category_ids/*")))[:NUM_TRAIN_IMAGES + NUM_VAL_IMAGES]

zipped_images = list(zip(images, masks))
random.shuffle(zipped_images)
# unzip
images, masks = list(zip(*zipped_images))
train_images, train_masks = list(images[:NUM_TRAIN_IMAGES]), list(masks[:NUM_TRAIN_IMAGES])
val_images = list(images[NUM_TRAIN_IMAGES:NUM_TRAIN_IMAGES + NUM_VAL_IMAGES])
val_masks = list(masks[NUM_TRAIN_IMAGES:NUM_TRAIN_IMAGES + NUM_VAL_IMAGES])
